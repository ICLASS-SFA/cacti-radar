...
def calc_3d_cellstats_singlefile(
    ...
    if os.path.isfile(ppi_filename):
        ...
        ds = ds.rename({'x': 'old_x', 'y': 'old_y'})
        ds = ds.assign_coords({'lat': dsv['y'].values,'lon': dsv['x'].values})
        ds = ds.rename({'lat': 'y', 'lon': 'x'})
        # Read variables
...
#         max_lwc = np.full((nmatchcloud, zdim), np.nan, dtype=np.float32)
        ...
        pct = [0,10,25,50,75,90,100] # Percentiles to calculate
        pdim = len(pct)
...
#                         import pdb;pdb.set_trace()
                        ...
                        # ivald1[imatchcloud,:] = np.sum(~np.isnan(sub_dbz), axis=(1,2))
                        # ivald2[imatchcloud,:] = np.sum(~np.isnan(sub_rwc), axis=(1,2))
            ...
            out_dict = {
                # nmatchcloud, 
                "cell_area": cell_area,
#                 "p10_reflectivity": pct_dbz[:,:,1],
#                 "p25_reflectivity": pct_dbz[:,:,2],
                "p50_reflectivity": pct_dbz[:,:,3],
#                 "p75_reflectivity": pct_dbz[:,:,4],
#                 "p90_reflectivity": pct_dbz[:,:,5],
                "max_reflectivity": pct_dbz[:,:,-1],
                "npix_dbz0": npix_dbz0,
                ...
                "p90_kdp": pct_kdp[:,:,5],
                "max_kdp": pct_kdp[:,:,-1],
#                 "p10_rainrate": pct_rainrate[:,:,1],
#                 "p25_rainrate": pct_rainrate[:,:,2],
                "p50_rainrate": pct_rainrate[:,:,3],
#                 "p75_rainrate": pct_rainrate[:,:,4],
#                 "p90_rainrate": pct_rainrate[:,:,5],
                "max_rainrate": pct_rainrate[:,:,-1],
#                 "p10_Dm": pct_Dm[:,:,1],
#                 "p25_Dm": pct_Dm[:,:,2],
                "p50_Dm": pct_Dm[:,:,3],
#                 "p75_Dm": pct_Dm[:,:,4],
...
#                 }, 
                ...
                'npix_dbz0': {
                    "long_name": "Number of pixels greater than 0 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz10': {
                    "long_name": "Number of pixels greater than 10 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz20': {
                    "long_name": "Number of pixels greater than 20 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz30': {
                    "long_name": "Number of pixels greater than 30 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz40': {
                    "long_name": "Number of pixels greater than 40 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz50': {
                    "long_name": "Number of pixels greater than 50 dBZ profile in a track",
                    "units": "counts",
                },
                'npix_dbz60': {
                    "long_name": "Number of pixels greater than 60 dBZ profile in a track",
                    "units": "counts",
                },
                'min_zdr': {
...
#                 },
                'max_rwc': {
                    "long_name": "Maximum rain water content profile in a track",
                    "units": "g/m^3",
                },
                'volrain': {
...
#     output_filename = f'{output_path}stats_3d_ppi_{startdate}_{enddate}.nc'
    output_filename = f'{output_path}stats_3d_ppi_percentiles_{startdate}_{enddate}.nc'
    ...
    match_ppibasetime = np.full(nfiles, np.NaN, dtype=float)
    for ifile in range(nfiles):
