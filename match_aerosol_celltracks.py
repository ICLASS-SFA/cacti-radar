...
if __name__ == '__main__':
    ...
    output_filename = f'{output_path}aerosol_celltrack_{startdate}_{enddate}.nc'
    ...
    aerosol_var_names = list(dsmup.data_vars.keys())
    # Add time to the list (since time is a coordinate, it is not included in the data_vars)
    aerosol_var_names.append('time')
    # Drop base_time, time_offset variables from the list
    aerosol_var_names.remove('base_time')
    aerosol_var_names.remove('time_offset')
    ...
    for tt in range(0, ntracks):
        ...
        matchindex[tt] = np.argmin(np.abs(aerosol_basetime - stats_basetime0[tt]))
        # Initiation time -1, -2, -3 hour 
        matchindex_1h[tt] = np.argmin(np.abs(aerosol_basetime[0:matchindex[tt]] - (stats_basetime0[tt] - one_hour*1))) 
        matchindex_2h[tt] = np.argmin(np.abs(aerosol_basetime[0:matchindex[tt]] - (stats_basetime0[tt] - one_hour*2)))
        matchindex_3h[tt] = np.argmin(np.abs(aerosol_basetime[0:matchindex[tt]] - (stats_basetime0[tt] - one_hour*3)))
        
        # Check if time difference is larger than defined time_window, if so save the track index
        if (aerosol_basetime[matchindex[tt]] - stats_basetime0[tt] > time_window):
            print(f'No match sonde time found: {stats_basetime0[tt]}')
            skip_tracks.append(tt)
            continue
        
        if (aerosol_basetime[matchindex_1h[tt]] - stats_basetime0[tt] + one_hour > time_window):
            print(f'No match sonde time (-1h) found: {stats_basetime0[tt]}')
            skip_tracks_1h.append(tt)
            continue
            
        if (aerosol_basetime[matchindex_2h[tt]] - stats_basetime0[tt] + 2*one_hour > time_window):
            print(f'No match sonde time (-2h) found: {stats_basetime0[tt]}')
            skip_tracks_2h.append(tt)
            continue
            
        if (aerosol_basetime[matchindex_3h[tt]] - stats_basetime0[tt] + 3*one_hour > time_window):
    ...
    gattrlist = {'title':  'Aerosol concentrations matched to tracked cells', \
                'Institution': 'Pacific Northwest National Laboratoy', \
                'Contact': 'Zhe Feng, zhe.feng@pnnl.gov', \
                'Created_on':  time.ctime(time.time()), \
                'source_track_file': trackstats_file, \
                'source_aerosol_file': aerosol_file, \
                'startdate': startdate, \
                'enddate': enddate, \
                }
